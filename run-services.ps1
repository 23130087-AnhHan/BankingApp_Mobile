param(
    [string]$JavaHome = $(if ($env:JAVA_HOME) { $env:JAVA_HOME } else { "C:\Program Files\Java\jdk-17" })
)
...
"@
        ...
        Start-ServiceWindow -ServiceName $service -WorkingDirectory $servicePath -BeforeCommand $beforeCommand -WaitSeconds 3
    } else {
